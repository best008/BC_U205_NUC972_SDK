...
export GCC_TYPE_DEFAULT=arm-none-linux-gnueabi
...
pack_module()
{
	...
	cd ${MODSRC_ROOT}
	tar czvf  ./${MODPKG_NAME} ${MODSRC_NAME}
	if [ $? -eq 0 ];then
		pdone "Tar source OK!"
	else
...
if [ "x$1" = "xbuild" ]; then
	check_env
	export CFLAGS="-I${MODSRC_PATH} -I${MODOUT_PATH}/include" 
